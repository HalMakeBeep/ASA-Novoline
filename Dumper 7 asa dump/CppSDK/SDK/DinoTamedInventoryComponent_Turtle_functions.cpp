#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: DinoTamedInventoryComponent_Turtle

#include "Basic.hpp"

#include "DinoTamedInventoryComponent_Turtle_classes.hpp"
#include "DinoTamedInventoryComponent_Turtle_parameters.hpp"


namespace SDK
{

// Function DinoTamedInventoryComponent_Turtle.DinoTamedInventoryComponent_Turtle_C.BPOverrideItemMinimumUseInterval
// (Event, Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// class UPrimalItem*                      theItem                                                (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, NoDestructor, HasGetValueTypeHash)
// float                                   ReturnValue                                            (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

float UDinoTamedInventoryComponent_Turtle_C::BPOverrideItemMinimumUseInterval(class UPrimalItem* theItem)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("DinoTamedInventoryComponent_Turtle_C", "BPOverrideItemMinimumUseInterval");

	Params::DinoTamedInventoryComponent_Turtle_C_BPOverrideItemMinimumUseInterval Parms{};

	Parms.theItem = theItem;

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}

}

