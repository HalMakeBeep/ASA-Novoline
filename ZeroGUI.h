...
namespace ZeroGUI
{
	...
	void Draw_Cursor(bool toggle)
	{
		if (toggle)
		{
			struct cursor_point { float x; float y; };

			auto draw_filled_poly = [&](const cursor_point* points, int count, fvector2d origin, float scale, flinearcolor color)
			{
				...
				{
					int py = (int)(origin.y + (points[i].y * scale));
				...
				{
					float intersections[16];
					int hit_count = 0;

					for (int i = 0; i < count; ++i)
					{
						const int j = (i + 1) % count;
						const float x1 = (float)origin.x + (points[i].x * scale);
						const float y1 = (float)origin.y + (points[i].y * scale);
						const float x2 = (float)origin.x + (points[j].x * scale);
						const float y2 = (float)origin.y + (points[j].y * scale);

						const bool crosses = ((y1 <= y && y2 > y) || (y2 <= y && y1 > y));
						if (!crosses) continue;
						if (hit_count >= 16) break;

						const float t = (float)(y - y1) / (y2 - y1);
						intersections[hit_count++] = x1 + (x2 - x1) * t;
					}
					...
					{
						canvas->k2_draw_line(
							fvector2d{ intersections[i], (float)y },
							fvector2d{ intersections[i + 1], (float)y },
							1.0f,
							color
						);
					}
			...
			auto draw_poly_outline = [&](const cursor_point* points, int count, fvector2d origin, float scale, flinearcolor color)
			{
				...
				for (int i = 0; i < count; ++i)
				{
					const int j = (i + 1) % count;
					canvas->k2_draw_line(
						fvector2d{ origin.x + points[i].x * scale, origin.y + points[i].y * scale },
						fvector2d{ origin.x + points[j].x * scale, origin.y + points[j].y * scale },
						1.0f,
						color
					);
				}
			...
			// Windows-style pointer silhouette (outline + fill).
			static const cursor_point outer[] = {
				{ 0.0f,  0.0f },
				{ 0.0f, 23.0f },
				{ 5.7f, 17.3f },
				{ 8.8f, 26.0f },
			...
			static const cursor_point inner[] = {
				{ 1.0f,  2.0f },
				{ 1.0f, 20.5f },
				{ 5.0f, 16.3f },
				{ 8.0f, 24.0f },
			...
			flinearcolor accent = Colors::Accent;
			flinearcolor border{
				accent.r * 0.35f,
				accent.g * 0.35f,
				accent.b * 0.35f,
			...
			flinearcolor shadow{ 0.0f, 0.0f, 0.0f, 0.28f };

			float hi_r = accent.r + 0.22f; if (hi_r > 1.0f) hi_r = 1.0f;
			float hi_g = accent.g + 0.22f; if (hi_g > 1.0f) hi_g = 1.0f;
			float hi_b = accent.b + 0.22f; if (hi_b > 1.0f) hi_b = 1.0f;
			flinearcolor highlight{ hi_r, hi_g, hi_b, 1.0f };

			// Soft drop shadow.
			draw_filled_poly(outer, 7, cursor_pos + fvector2d{ 1.0f, 1.0f }, scale, shadow);

			// Pointer body.
			draw_filled_poly(outer, 7, cursor_pos, scale, border);
			draw_filled_poly(inner, 7, cursor_pos, scale, fill);
			draw_poly_outline(outer, 7, cursor_pos, scale, border);

			// Highlight edge similar to Windows cursor shading.
			canvas->k2_draw_line(cursor_pos + fvector2d{ 1.8f, 2.3f }, cursor_pos + fvector2d{ 6.8f, 13.8f }, 1.0f, highlight);
			canvas->k2_draw_line(cursor_pos + fvector2d{ 2.7f, 2.9f }, cursor_pos + fvector2d{ 7.7f, 14.3f }, 1.0f, highlight);
		}
	}
	...
	void Combobox(const char* name, fvector2d size, int* value, const char* arg, ...)
	{
		...
		for (va_start(arguments, arg); arg != NULL; arg = va_arg(arguments, const char*))
		{
			...
			if (checkbox_enabled[elements_count])
			{
				...
				{
					hover_element = true;
					PostRenderer::drawFilledRect(fvector2d{ element_pos.x, element_pos.y }, size.x, 25.0f, Colors::Combobox_Elements);
